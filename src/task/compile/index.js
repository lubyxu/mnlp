...
const registerCompileTask = () => {
  ...
  task('compile', done => {
    ...
    merge2([
      compileLess(src('src/**/*.less', {cwd: process.cwd()}).pipe(new plumber())),
      ...
      src(['src/**/*.svg', 'src/**/*.bmp', 'src/**/*.gif', 'src/**/*.jpe?g', 'src/**/*.png']).pipe(new plumber())
    ]).pipe(dest(program.opts().outDir || 'dest/')).on('finish', function () {
